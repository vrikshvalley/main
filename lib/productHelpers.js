...
export async function getProducts(options = {}) {
  ...
  const PLANT_DOCTOR_PRODUCT = {
    ...
    price: 49,
    description:
      "Book a 30-minute expert plant consultation. Get personalized diagnosis, care plan and recommendations.",
  ...
  if (searchQuery && searchQuery.trim()) {
    const searchResult = await searchProductsFromDataSource(searchQuery, {
      category: categoryName,
      pageSize: limit,
    });

    let searchProducts = searchResult.data || [];

    // Add Plant Doctor if search matches
    if (
      searchQuery.toLowerCase().includes("doctor") ||
      searchQuery.toLowerCase().includes("consultation")
    ) {
      searchProducts.push(PLANT_DOCTOR_PRODUCT);
    }

    // Apply price filter client-side on search results
    if (priceRange) {
    ...
    const paginatedProducts = searchProducts.slice(
      startIndex,
      startIndex + limit
    );
  ...
  const result = await getProductsFromDataSource({
    ...
    sortBy: mappedSortBy,
    page: 1,
    pageSize: 1000, // Fetch all products without pagination at service level
    featured,
    // Default to showing all products; only filter when explicitly requested
    inStock: inStock === null ? null : inStock,
  });

  let products = result.data || [];

  // Add Plant Doctor to Plant Care subcategory
  if (subcategory && subcategory.toLowerCase() === "plant care") {
    products = [PLANT_DOCTOR_PRODUCT, ...products];
  }
  ...
  // Apply inStock filter
  if (inStock) {
    products = products.filter((p) => (p.stock || p.quantity || 0) > 0);
  }

  // Calculate proper pagination - count the filtered products as the total
