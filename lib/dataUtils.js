...
export function getCategoriesFromJSON() {
  ...
  const normalizeCategoryImage = (p) => {
    if (!p) return "";
    if (typeof p !== "string") return "";
    if (p.startsWith("http")) return p;
    if (p.startsWith("/categories/")) return p;
    if (p.startsWith("/")) return `/categories/${p.replace(/^\//, "")}`;
    return `/categories/${p}`;
  };

  const normalizeSubcategoryImage = (p) => {
    if (!p) return "";
    if (typeof p !== "string") return "";
    if (p.startsWith("http")) return p;
    if (p.startsWith("/subcategories/")) return p;
    if (p.startsWith("/")) return `/subcategories/${p.replace(/^\//, "")}`;
    return `/subcategories/${p}`;
  };
