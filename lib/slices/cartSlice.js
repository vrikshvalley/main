...
export const loadUserCart = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      const cart = await loadCartFromFirebase(userId);
      return cart;
    } catch (error) {
...
export const addItemAsync = createAsyncThunk(
  ...
  async ({ item, userId }, { rejectWithValue }) => {
    try {
      if (userId) {
        await addToCartFirebase(userId, item.id, item.qty ?? 1);
      }
      return { item, userId };
    } catch (error) {
      return rejectWithValue(error.message);
...
export const updateItemAsync = createAsyncThunk(
  ...
  async ({ productId, quantity, userId }, { rejectWithValue }) => {
    try {
      if (userId) {
        await updateCartItemFirebase(userId, productId, quantity);
      }
...
export const removeItemAsync = createAsyncThunk(
  ...
  async ({ productId, userId }, { rejectWithValue }) => {
    try {
      if (userId) {
        await removeFromCartFirebase(userId, productId);
      }
...
export const clearCartAsync = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      if (userId) {
        await clearCartFromFirebase(userId);
      }
      return { userId };
    } catch (error) {
      return rejectWithValue(error.message);
...
const cartSlice = createSlice({
  ...
  reducers: {
    addItem: (state, action) => {
      ...
      const existing = state.items[sanitizedItem.id];
      if (existing) {
        existing.qty += sanitizedItem.qty ?? 1;
      } else {
        state.items[sanitizedItem.id] = {
          ...sanitizedItem,
          qty: sanitizedItem.qty ?? 1,
        };
      }
