...
export async function authenticate() {
  // Return cached token if still valid (with 1-day buffer before 10-day expiry)
  if (cachedToken && tokenExpiry && Date.now() < tokenExpiry) {
    const daysLeft = Math.floor(
      (tokenExpiry - Date.now()) / 1000 / 60 / 60 / 24
    );
    const hoursLeft =
      Math.floor((tokenExpiry - Date.now()) / 1000 / 60 / 60) % 24;
    console.log(
      `✅ Using cached Shiprocket token (expires in ${daysLeft}d ${hoursLeft}h)`
    );
    return cachedToken;
  }

  if (cachedToken && tokenExpiry) {
    console.log("⚠️ Shiprocket token expired, refreshing...");
  }
  ...
  try {
    ...
    // Token valid for 10 days, cache for 9 days to ensure refresh before expiry
    ...
    console.log(
      `📅 Token expires at: ${expiryDate.toISOString()} (in ${daysUntilExpiry} days)`
    );
    console.log(`🔒 Token cached and will auto-refresh after 9 days`);
