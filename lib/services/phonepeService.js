...
// Production URLs (change to sandbox URLs for testing)
...
export const authenticate = async () => {
  try {
    ...
    if (cachedToken && tokenExpiry && Date.now() < tokenExpiry) {
      const timeLeft = Math.floor((tokenExpiry - Date.now()) / 1000 / 60);
      console.log(
        `✅ Using cached PhonePe token (expires in ${timeLeft} minutes)`
      );
      return { data: cachedToken, error: null };
    }

    if (cachedToken && tokenExpiry) {
      console.log("⚠️ PhonePe token expired, refreshing...");
    }

    console.log("🔄 Generating new PhonePe auth token...");
    console.log("Auth URL:", PHONEPE_AUTH_URL);
    console.log("Client ID:", PHONEPE_CLIENT_ID?.substring(0, 10) + "...");
    ...
    // Add client_version only if it exists
    if (PHONEPE_CLIENT_VERSION) {
      requestData.client_version = PHONEPE_CLIENT_VERSION;
    }

    console.log("Request params:", Object.keys(requestData));
    ...
    // Set expiry 5 minutes before actual expiry for safety (expires_at is in seconds)
    tokenExpiry = expires_at * 1000 - 5 * 60 * 1000;

    const expiryDate = new Date(tokenExpiry);
    const expiresInMinutes = Math.floor((tokenExpiry - Date.now()) / 1000 / 60);

    console.log("✅ PhonePe auth token generated successfully");
    console.log(
      `📅 Token expires at: ${expiryDate.toISOString()} (in ${expiresInMinutes} minutes)`
    );
    console.log(`🔒 Token cached and will auto-refresh before expiry`);
