...
/**
 * Order Service
 * Handles order CRUD operations with Supabase
 * Integrates with Shiprocket for shipping and tracking
 */
...
export async function createOrder(orderData) {
  try {
    const order = {
      ...
      payment_method: orderData.paymentMethod || "phonepe",
      phonepe_merchant_order_id: orderData.phonepeMerchantOrderId || null,
      phonepe_order_id: orderData.phonepeOrderId || null,
      phonepe_transaction_id: orderData.phonepeTransactionId || null,

      // Shiprocket details
      shiprocket_order_id: orderData.shiprocketOrderId || null,
      shiprocket_shipment_id: orderData.shiprocketShipmentId || null,
    ...
    console.log("Order created in database:", data[0].order_id);
 ...
 */
export async function getUserOrders(userId, limit = null, status = null) {
  try {
    let query = supabase
      .from("orders")
      .select("*")
 ...
 * @param {Object} paymentDetails - PhonePe payment details
 * @returns {Promise<Object>} Updated order
 */
 ...
 * @param {string} orderId - Order ID
 * @param {Object} shippingDetails - Shiprocket shipping details
 * @returns {Promise<Object>} Updated order
 */
 ...
 * @returns {Array} Formatted items for Shiprocket API
 */
export function formatItemsForShiprocket(cartItems) {
