...
import { searchProducts as searchFirestoreProducts, getSearchSuggestions as getFirestoreSuggestions } from './services/productService';
...
export const getSearchSuggestions = async (query, limit = 5) => {
  ...
  try {
    const { data, error } = await getFirestoreSuggestions(query, limit);
